...
import { HTTPRequest, ConnectorError, DataSourceOperation } from '../../connectors/Connector';
...
var GraphQLConnector = /** @class */ (function (_super) {
    ...
    GraphQLConnector.prototype.transformResponse = function (response, request) {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                ...
                fetchFieldName = inflection.camelize(request.operation === DataSourceOperation.list
                    ? inflection.pluralize(request.name)
                    : request.name, true);
                data = response.data && response.data[fetchFieldName];
                if (null === data) {
    ...
    GraphQLConnector.prototype.transformRequest = function (request) {
        ...
        var entityName = inflection.camelize(request.name, false);
        ...
        var query = new GraphQLQuery(request.operation === 'read' || request.operation === 'list'
            ? 'query'
            : 'mutation', this.generateQueryParams(fetchFieldName, args));
    ...
    GraphQLConnector.prototype.generateQueryParams = function (name, args) {
        var str = 'operation';
        var header = Object
            .keys(args)
            .map(function (key) {
            switch (key) {
                case 'filter':
                    if (!args[key])
                        return '';
                    return "$" + key + ": " + inflection.singularize(inflection.camelize(name, false)) + "FilterType";
                case 'sort':
                    if (!args[key])
                        return '';
                    return "$" + key + ": [" + inflection.singularize(inflection.camelize(name, false)) + "SortType!]";
                default:
                    return '';
            }
        })

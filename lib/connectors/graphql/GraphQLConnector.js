        ...
        var header = Object
            ...
            switch (key) {
                case 'filter':
                    if (!args[key])
                        return '';
                    return "$" + key + ": " + inflection.singularize(inflection.camelize(fieldName, false)) + "FilterType";
                case 'sort':
                    if (!args[key])
                        return '';
                    return "$" + key + ": [" + inflection.singularize(inflection.camelize(fieldName, false)) + "SortType!]";
                case 'input':
                    if (!args[key])
                        return '';
                    return "$" + key + ": " + inflection.singularize(inflection.camelize(request.name, false)) + "Raw" + inflection.camelize(request.operation) + "Input!";
                default:
                    return '';
            }
        })
